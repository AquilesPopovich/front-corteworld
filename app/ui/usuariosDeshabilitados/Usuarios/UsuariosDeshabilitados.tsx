        ...
        <div className={`fixed flex flex-row flex-wrap justify-start gap-4 items-center h-3/4 w-3/4 left-80 bottom-10 border-solid rounded-lg bg-slate-50 p-4 overflow-y-auto ${styles.container}`}>
            {usuarios?.map((user) => (
                <div key={user.id} className={`flex flex-col items-center justify-evenly border-2 rounded-md border-black bg-gray-100 h-4/6 w-1/4 ${styles.cardUser}`}>
                    ...
                    {user.status ? (<h3 className={`font-medium`}>Estado: Verdadero</h3>) : (<h3 className={`font-medium`}>Estado: Falso</h3>)}
                    <button className={`bg-pink-200 rounded-md hover:bg-pink-400`} onClick={() => habilitarUser('yes', `${user.id}`)}>Habilitar Usuario</button>
                    <button className={`bg-pink-200 rounded-md hover:bg-pink-400`} onClick={() => habilitarUser('no', `${user.id}`)}>Deshabilitar Usuario</button>
                    <button className={`bg-pink-200 rounded-md hover:bg-pink-400`} onClick={() => setComentarios([user.id])}>Ver Comentarios</button>
                    {comentarios.includes(user.id) && (
                        <ComentariosUsers userId={user.id} comentarios={comentarios} setComentarios={setComentarios} />
                    )}
                </div>
            ))}
            <button className={`bg-pink-200 rounded-md hover:bg-pink-400`} onClick={() => setMostrarUsuarios(false)}>Cancelar</button>
        </div>
    )
}
